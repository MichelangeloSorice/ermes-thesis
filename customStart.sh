...
## Added to avoid falsy data to be impossible to delete
if [ -d './results' ]; then
    rm -rf './results'
fi
...
if [ -z "${RESULT_DIR}" ]; then
    ...
    ffmpeg -i ${RESULT_DIR}/video/${videoFile} -vf fps=10 ${RESULT_DIR}/video/capture%05d.jpg -preset superfast -y -hide_banner -loglevel 16
    rm ${RESULT_DIR}/video/${videoFile}
