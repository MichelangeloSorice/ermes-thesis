...
app.reload = function () {chrome.runtime.reload()};
app.version = function () {return chrome.runtime.getManifest().version};
app.homepage = function () {return chrome.runtime.getManifest().homepage_url};
app.toolbarIcon = function (e) {chrome.browserAction.setIcon({"path": "../../data/icons/32-" + e + ".png"})};
...
var onRemovedListener = function (tabId) {delete app.tabsObject[tabId]};
var onCreatedListener = function (tab) {if (tab) app.tabsObject[tab.id] = tab};
...
var load = function (e) {
  ...
  /*  */
  if (flag_WRB) chrome.webRequest.onBeforeRequest.addListener(onBeforeRequest, {"urls": ['<all_urls>'], "types": types}, ["blocking"]);
  if (flag_CSP) chrome.webRequest.onHeadersReceived.addListener(onHeadersReceived, {"urls": ['<all_urls>'], "types": ["main_frame", "sub_frame"]}, ["blocking", "responseHeaders"]);
};
...
app.storage = (function () {
  ...
  return {
    ...
    "clear": function (callback) {chrome.storage.local.clear(callback)},
...
app.tab = {
  "onUpdated": function (callback) {onUpdated = callback},
  "onActivated": function (callback) {onActivated = callback},
  "open": function (url) {chrome.tabs.create({"url": url, "active": true})},
  "openOptions": function () {chrome.runtime.openOptionsPage(function () {})},
  "getActive": function (callback) {chrome.tabs.query({"currentWindow": true, "active": true}, function (e) {if (e && e.length) callback(e[0])})}
};
  ...
  if (CSP) {
    for (var i = 0; i < headers.length; i++) {
      ...
      if (name === "content-security-policy") {
        headers[i].value = CSP + headers[i].value; /* add to existing CSP */
...
chrome.tabs.query({}, function (e) {e.forEach(function (tab) {if (tab) app.tabsObject[tab.id] = tab})});
