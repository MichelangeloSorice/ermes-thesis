{
  ...
  const build = json => {
    ...
    /*  */
    const n = Math.floor(parseInt(window.getComputedStyle(document.body).width) / 150) || 3;
    json.slice(0, n).forEach(({id, title}, index) => {
      ...
      var tmp = title.replace(' -', '').split(' ');
      icon.textContent = tmp[0].charAt(0).toUpperCase() + tmp[1].charAt(0).toUpperCase();
