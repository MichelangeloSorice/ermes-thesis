...
µBlock.scriptletFilteringEngine = (function () {
    ...
    api.retrieve = function (request) {
        if (scriptletDB.size === 0) {
            return;
        }
        if (µb.hiddenSettings.ignoreScriptInjectFilters) {
            return;
        }
    ...
    api.injectNow = function (details) {
        ...
        if (µb.URI.isNetworkURI(details.url) === false) {
            return;
        }
        let request = {
            tabId: details.tabId,
