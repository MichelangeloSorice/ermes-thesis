...
//   Support the seamless loading of sublists.
    ...
    api.fetchFilterList = function (mainlistURL, onLoad, onError) {
        ...
        var processIncludeDirectives = function (details) {
            ...
            for (; ;) {
                ...
                if (toParsedURL(match[1]) !== undefined) {
                    continue;
                }
                ...
                if (pendingSublistURLs.has(subURL)) {
                    continue;
                }
                if (loadedSublistURLs.has(subURL)) {
                    continue;
                }
    ...
    var registerAssetSource = function (assetKey, dict) {
        ...
        for (var prop in dict) {
            if (dict.hasOwnProperty(prop) === false) {
                continue;
            }
    ...
    var updateAssetSourceRegistry = function (json, silent) {
        ...
        if (newDict instanceof Object === false) {
            return;
        }
        ...
        var onReady = function () {
            ...
            for (var assetKey in cacheDict) {
                if (pattern instanceof RegExp) {
                    if (pattern.test(assetKey) === false) {
                        continue;
                    }
                    ...
                    if (pattern.indexOf(assetKey) === -1) {
                        continue;
                    }
                }
                if (exclude instanceof RegExp) {
                    if (exclude.test(assetKey)) {
                        continue;
                    }
                    ...
                    if (exclude.indexOf(assetKey) !== -1) {
                        continue;
                    }
                }
                cacheEntry = cacheDict[assetKey];
                if (!cacheEntry.writeTime) {
                    continue;
                }
...
// Asset updater area.
    ...
    var updateNext = function () {
        ...
        var findOne = function () {
            ...
            for (var assetKey in assetDict) {
                assetEntry = assetDict[assetKey];
                if (assetEntry.hasRemoteURL !== true) {
                    continue;
                }
                if (updaterFetched.has(assetKey)) {
                    continue;
                }
                cacheEntry = cacheDict[assetKey];
