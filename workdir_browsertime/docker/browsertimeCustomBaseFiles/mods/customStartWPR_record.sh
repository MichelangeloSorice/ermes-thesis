...
function runWebPageReplay() {
  ...
  LATENCY=${LATENCY:-100}
  sudo mkdir -p /tmp/recordsFolder
  WPR_PARAMS="--http_port $WPR_HTTP_PORT --https_port $WPR_HTTPS_PORT --https_cert_file $CERT_FILE --https_key_file $KEY_FILE --inject_scripts $SCRIPTS /tmp/recordsFolder/archive_$replay_iteration.wprgo"
  WAIT=${WAIT:-5000}
...
chromeSetup
setupADB
## Should simply set an env variable telling how many times we should run recordings
...
while [ $replay_iteration -lt $limit ]; do
   ...
   echo "$@"
   # Here we are preparing a fresh copy of google chrome profile data
   #rm -rf /tmp/config/google-chrome
   #mkdir -p /tmp/config/google-chrome
   #cp -R /config/google-chrome /tmp/config/
   ls /tmp/rep
   runWebPageReplay "$@"
